"""
Sourcegraph Auth Perms Sync uses metadata from auth providers to set:
- Explicit repo permissions
- Organizations and memberships

See https://github.com/sourcegraph/auth-perms-sync/blob/main/README.md for usage instructions
...
class AuthPermissionsSyncConfig(src.SourcegraphClientConfig, src.LoggingConfig):
    """Config values loaded from defaults, .env, environment, and CLI flags."""

    get: bool = src.config_field(
        default=False,
        env_var="AUTH_PERMS_SYNC_GET",
        cli_flag="--get",
    ...
    set_path: Path | None = src.config_field(
        ...
        help=(
            ...
            "Defaults to maps.yaml under auth-perms-sync-runs/<endpoint>/.\n"
            "Relative paths are resolved from that path."
        ),
    )
    full: bool = src.config_field(
        default=False,
        env_var="AUTH_PERMS_SYNC_FULL",
        cli_flag="--full",
        cli_action="store_true",
    ...
    users_without_explicit_perms: bool = src.config_field(
        default=False,
        env_var="AUTH_PERMS_SYNC_USERS_WITHOUT_EXPLICIT_PERMS",
    ...
    created_after: str | None = src.config_field(
        default=None,
        env_var="AUTH_PERMS_SYNC_CREATED_AFTER",
    ...
    restore_path: Path | None = src.config_field(
        default=None,
        env_var="AUTH_PERMS_SYNC_RESTORE",
        cli_flag="--restore",
        metavar="FILE",
        help=(
            "Restore explicit-permissions state to match the given snapshot JSON file.\n"
            "Relative paths are resolved under 'auth-perms-sync-runs/<endpoint>/.'"
        ),
    )
    sync_saml_organizations: bool = src.config_field(
        default=False,
        env_var="AUTH_PERMS_SYNC_SYNC_SAML_ORGS",
    ...
    parallelism: int = src.config_field(
        default=16,
        env_var="AUTH_PERMS_SYNC_PARALLELISM",
    ...
    max_attempts: int = src.config_field(
        default=5,
        env_var="AUTH_PERMS_SYNC_MAX_ATTEMPTS",
    ...
    no_backup: bool = src.config_field(
        default=False,
        env_var="AUTH_PERMS_SYNC_NO_BACKUP",
        cli_flag="--no-backup",
        cli_action="store_true",
    ...
    sample_interval: float = src.config_field(
        default=10.0,
        env_var="AUTH_PERMS_SYNC_SAMPLE_INTERVAL",
...
def config_error(message: str) -> NoReturn:
    ...
    print(f"auth-perms-sync: error: {message}", file=sys.stderr)
    raise SystemExit(2)


def validate_config(config: AuthPermissionsSyncConfig) -> None:
...
def validate_command_selection(config: AuthPermissionsSyncConfig) -> None:
...
def validate_user_filter_selection(config: AuthPermissionsSyncConfig) -> None:
...
def validate_set_mode_selection(config: AuthPermissionsSyncConfig) -> None:
...
def set_command_options(config: AuthPermissionsSyncConfig) -> permission_types.SetCommandOptions:
...
def resolve_command(config: AuthPermissionsSyncConfig) -> ResolvedCommand:
...
def resolve_set_command(config: AuthPermissionsSyncConfig, run_mode: str) -> ResolvedCommand:
...
def load_config() -> AuthPermissionsSyncConfig:
    """Parse and validate CLI/environment config."""
    config = src.parse_args(
        AuthPermissionsSyncConfig,
        description=__doc__,
        base_dir=Path("."),
    )
...
def endpoint_scoped_config(
    config: AuthPermissionsSyncConfig, endpoint: str
) -> AuthPermissionsSyncConfig:
...
def require_set_input_file(config: AuthPermissionsSyncConfig) -> None:
    ...
    raise SystemExit(
        ...
        f"{config.set_path}\n"
        "Run `uv run auth-perms-sync --get` to create the default maps.yaml, "
        "or pass a path to an existing maps file."
    )


def run_fields(
    config: AuthPermissionsSyncConfig, command: ResolvedCommand, endpoint: str
) -> dict[str, object]:
...
def run_with_client(
    config: AuthPermissionsSyncConfig,
    ...
    http = src.HTTPClient(
        user_agent="auth-perms-sync/0.1 (+python)",
        max_attempts=config.max_attempts,
